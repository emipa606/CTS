	...
	public class PlasmaBigGen : Building
	{
		...
		static PlasmaBigGen()
		{
			checked
			{
				...
				{
                    TexResFrames[i] = GraphicDatabase.Get<Graphic_Single>(FramePath + (i + 1), ThingDef.Named("CTSPlasmaBigGen").graphicData.Graphic.Shader);
                    TexResFrames[i].drawSize = ThingDef.Named("CTSPlasmaBigGen").graphicData.drawSize;
				}
                TexResFrames2 = new Graphic_Single[FrameCount];
				for (var i = 0; i < FrameCount; i++)
				{
                    TexResFrames2[i] = GraphicDatabase.Get<Graphic_Single>(FramePath2 + (i + 1), ThingDef.Named("CTSPlasmaBigGen").graphicData.Graphic.Shader);
